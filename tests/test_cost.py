...
from ccmeter.report import PRICING, cost_usd, pricing_for


def test_known_models_resolve_to_pricing():
    """Every model prefix in PRICING must resolve via pricing_for."""
    for model in PRICING:
        assert pricing_for(model) is PRICING[model]


def test_unknown_model_falls_back_to_opus():
    """Unknown models default to opus pricing (most expensive = conservative)."""
    rates = pricing_for("claude-unknown-99")
    assert rates is PRICING["claude-opus-4-6"]


def testcost_usd_pure_input():
    """1M input tokens on opus = $5.00."""
    tokens = {"input": 1_000_000, "output": 0, "cache_read": 0, "cache_create": 0}
    assert cost_usd(tokens, "claude-opus-4-6") == 5.00


def testcost_usd_pure_output():
    """1M output tokens on opus = $25.00."""
    tokens = {"input": 0, "output": 1_000_000, "cache_read": 0, "cache_create": 0}
    assert cost_usd(tokens, "claude-opus-4-6") == 25.00


def testcost_usd_cache_read_is_cheap():
    ...
    cost = cost_usd(tokens, "claude-opus-4-6")
    assert cost == 0.50
...
def test_cache_read_ratio_matters():
    ...
    cache_cost = cost_usd(heavy_cache, "claude-opus-4-6")
    input_cost = cost_usd(heavy_input, "claude-opus-4-6")
...
def test_sonnet_cheaper_than_opus():
    ...
    opus_cost = cost_usd(tokens, "claude-opus-4-6")
    sonnet_cost = cost_usd(tokens, "claude-sonnet-4-6")
